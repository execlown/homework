...
function FormCreatePlaylist() {
  ...
  return (
    <Form noValidate validated={validated} onSubmit={handleSubmit} className="w-50 m-auto mt-4" >
      <Form.Group className="mb-4">
        <Form.Label className="text-white fs-4">Playlist Title</Form.Label>
        <Form.Control type="text" value={title} onChange={handleTitle} required minLength={10} />
        <Form.Control.Feedback type="invalid" className="text-center fs-6">
          {errMsg}
        </Form.Control.Feedback>
      </Form.Group>
      <Form.Group>
        <Form.Label className="text-white fs-4"> Playlist Description </Form.Label>
        <Form.Control as="textarea" value={description} onChange={(e) => setDescription(e.target.value)} style={{ resize: 'none', height: '10rem' }} />
      </Form.Group>
